...
public class NMapCalloutCustomOldOverlay extends NMapCalloutOverlay {
  ...
  private void adjustTextBounds(NMapView mapView) {
    ...
    if (mTitleTruncated == null || mWidthTitleTruncated != mapViewWidth) {
      ...
      final String title =
          TextUtils.ellipsize(
                  mOverlayItem.getTitle(), mTextPaint, maxWidth, TextUtils.TruncateAt.END)
              .toString();
      ...
      if (DEBUG) {
        Log.i(
            LOG_TAG,
            "adjustTextBounds: mTitleTruncated="
                + mTitleTruncated
                + ", length="
    ...
    if (DEBUG) {
      Log.i(
          LOG_TAG,
          "adjustTextBounds: mTempRect.width="
              + mTempRect.width()
              + ", mTempRect.height="
              + mTempRect.height());
    }
